...
/*
* 名称 ： getOffset(ele,attr)
* 功能： 获取元素距离窗口边缘的位置
* 参数：
* ele: 需要获取属性的元素  （不能是集合）
* attr: 属性名称 （只有两个值： offsetTop ,offsetLeft）
*
* */
function getOffset(ele,attr){
    var total = 0;
    function deepCount(ele,attr){

        var t = ele[attr];

        total += t;

        var oParent = ele.offsetParent;//元素


        if(oParent&&oParent.nodeName.toLowerCase()!='body'){
            deepCount(oParent,attr);
        }
...
//        };
    ...
    box.onmousedown = function(ev){
        ...
        document.onmousemove = function(ev){
            ...
            //边界处理：
            if(x2<getOffset(box,'offsetLeft')+box.clientLeft){
                x = x1-(getOffset(box,'offsetLeft')+box.clientLeft);
            }
            if(x2>getOffset(box,'offsetLeft')+box.clientWidth){
                x = getOffset(box,'offsetLeft')+box.clientWidth-x1;
            }

            if(y2<getOffset(box,'offsetTop')+box.clientTop){
                y = y1-(getOffset(box,'offsetTop')+box.clientTop);
            }
            if(y2>getOffset(box,'offsetTop')+box.clientHeight){
                y = getOffset(box,'offsetTop')+box.clientHeight-y1;
            }
